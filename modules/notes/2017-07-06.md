...
### Discussion:
* puppet-agent 5.0.0 is out! beaker-puppet still can't install it (see https://github.com/puppetlabs/beaker-puppet/pull/6) and thus beaker-puppet_install_helper can't install it (see puppet-dev@ mailing list)
* because of semantic-puppet, puppetlabs_spec_helper borks on it: https://tickets.puppetlabs.com/browse/MODULES-5155
* We are just going to ignore it for a couple weeks until 5.0.1 comes out.
* Talked about evaluate_generators in https://github.com/puppetlabs/puppet/blob/master/lib/puppet/parser/compiler.rb
* Talked about https://github.com/github/hub which adds `git checkout $pr_url` and `git merge $pr_url`
* Soffleés collapse, deal with it.
